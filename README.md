...
## Информация о дополнениях akumidv
1. Сделан файл install.cmd - для установки под windows
2. Подправлены библиотеки на новые
3. Чтобы запустить нужно брать /tmp из докера (см. инструкцию к компанде `docker cp`), там же находятся 
   чекпоинт torch модели читчата (болталки). 
   Это модель обученная по совместимости GPT2 через фреймворк transformer, можно и свою обучить. 
   Положить нужно `./tmp/rugpt_checkpoints` 
4. Подключена модель ruGPT3Small обученная по модели dialoGPT микрософта. Положить нужно в папку    
. Чтобы запустить, нужно положить файлы чекпоинта `./tmp/dialogptru_checkpoints`
5. Чат и установки все собраны в нотебук под google collab (пути для скачивания архивов надо свои прописать): 
   файл `koziev-chatbot-dialogpt.ipynb`
6. Подчищены правила и сценарии на предмет удаления эмодзи - проблема с их загрузкой под windows (не стал разбираться)

Итоговая цель обернуть модель GPT3 сбербанка, затюненную по методике DialoGPT на предметную область, готовым чатботом,
в котором реализованы правила, сцеанрии и база фактов.

TODO:
- надо перед енкодером-декодером добавить спеллер яндекса, т.к. ошибки исправляются не очень хорошо
- надо изменить приоритетность выбора ответов, в т.ч. между ответами читчата и dialogpt.
- надо менять правила и сценарии по предметку обученной модели dialogpt, читчат активировать, если совсем ответов нет хороших.
- dialogpt3 лучше обучить на Large модель, доступную для скачивания
- можно вместо чит-чата или ещё одним генератором подключить сервис GPT3 XL сбербанка
- все медленные генераторы (GPT3) лучше запускать асинхронно.  
- надо оптимизировать стандартные параметры GPT3 модели (температуру и т.д. другие параметры, для более коротких ответов)
- можно обернуть воис декодером и синтезом.
